...
def estimate(fl, slow_motion_factor=1):
    ...
    csv_file_path = f'results_wrist_elbow_shoulder/variables_{fl}.csv'
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        if view == 'front':
            writer.writerow(["timestamp", 'right_wrist_x', 'right_wrist_y','left_wrist_x', 'left_wrist_y','left_elbow_x', 'left_elbow_y','right_elbow_x', 'right_elbow_y','left_shoulder_x','left_shoulder_y'])
        elif view == 'side':
            writer.writerow([])
    ...
    out = cv2.VideoWriter(f'results_wrist_elbow_shoulder/{fl}_out_{timestamp}.mp4', cv2.VideoWriter_fourcc(*'MP4V'), fps, (frame_width, frame_height))
                ...
                try:
                    ...
                    if view == 'front':
                        ...
                        with open(csv_file_path, mode='a', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerow([video_timestamp, right_wrist.x * w, right_wrist.y * h,left_wrist.x * w, left_wrist.y * h,left_elbow.x * w, left_elbow.y * h,right_elbow.x * w, right_elbow.y * h,left_shoulder.x * w, left_shoulder.y * h])

                        cv2.circle(duplicated_image, (int(right_wrist.x * w), int(right_wrist.y * h)), 6, (255, 0, 255), -1)
                        cv2.circle(duplicated_image, (int(left_wrist.x * w), int(left_wrist.y * h)), 6, (255, 255, 0), -1)
                        cv2.circle(duplicated_image, (int(left_elbow.x * w), int(left_elbow.y * h)), 6, (0, 255, 255), -1)
                        cv2.circle(duplicated_image, (int(right_elbow.x * w), int(right_elbow.y * h)), 6, (250, 0, 250), -1)
                        cv2.circle(duplicated_image, (int(left_shoulder.x * w), int(left_shoulder.y * h)), 6, (250, 0, 250), -1)
